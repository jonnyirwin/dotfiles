...
LOCK_FILE="/tmp/pomodoro_break_window.lock"
if [ -f "$LOCK_FILE" ]; then
...
cleanup() {
    rm -f "$LOCK_FILE"
    rm -f "/tmp/pomodoro_break_cancel"
    exit
}
...
create_floating_break() {
    local temp_script="/tmp/pomodoro_break_display.sh"
...
while true; do
    ...
    TIMER_STATE=$(~/.config/waybar/scripts/pomodoro.sh display 2>/dev/null)
    ...
    case "$input" in
        'q'|'Q')
            ...
            ~/.config/waybar/scripts/pomodoro.sh skip-break
            break
            ;;
        's'|'S')
            # Stop completely - communicate back to main timer
            ~/.config/waybar/scripts/pomodoro.sh stop
            break
            ;;
        '+'|'=')
            # Extend break - communicate back to main timer
            ~/.config/waybar/scripts/pomodoro.sh extend-break
...
~/.config/waybar/scripts/pomodoro.sh finish-break
...
if command -v zenity >/dev/null 2>&1; then
    ...
    CANCEL_FLAG="/tmp/pomodoro_break_cancel"
    rm -f "$CANCEL_FLAG"
