...
reload_services() {
    ...
    # kitty — try the remote-control socket first (instant resize for whichever
    # kitty holds the abstract `@kitty` socket), then SIGUSR1 every running kitty
    # to reload its config. The signal is universal: every kitty process picks up
    # the new font_size we just wrote to kitty.conf, regardless of socket state.
    command -v kitty >/dev/null 2>&1 && \
        kitty @ --to unix:@kitty set-font-size --all "$kitty_size" >/dev/null 2>&1 || true
    pkill -USR1 -x kitty 2>/dev/null || true

    # waybar (Wayland) — SIGUSR2 triggers a full reload.
    pkill -SIGUSR2 waybar 2>/dev/null || true

    # polybar (X11) — relaunch cleanly via the existing launch script.
